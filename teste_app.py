...
def pegar_datas():
    ...
    start_date = now.replace(day=now.day - 1 if now.day > 1 else 1, hour=0, minute=0, second=0, microsecond=0)
...
def msg_dif(cor, lugar, mensagem):
    linha = '-' * len(mensagem)
    if lugar == 'cima':
        print(colored(f'{linha}\n{mensagem}', cor))
    elif lugar == 'baixo':
        print(colored(f'{mensagem}\n{linha}', cor))
    elif lugar == 'ambos':
        print(colored(f'{linha}\n{mensagem}\n{linha}', cor))
    else:
...
def fazer_post(id_mlb, tv1, tv2):
    url_id = f'{BASE_URL}/items/{id_mlb}'
    corpo_produto = fazer_reqs(url_id, tv1)

    url_id = f'{BASE_URL}/items'
...
def configurar_conta():
    ...
    while not conta_configurada:
        ...
        headers = {'Authorization': f'Bearer {tv}'}
        resposta = requests.get(f'{BASE_URL}/users/me', headers=headers)
...
def nome_conta(tv):
    nome_retorno = fazer_reqs(f'{BASE_URL}/users/me', tv)
    conta = nome_retorno['nickname']
    return conta


def id_conta(tv):
    id_retorno = fazer_reqs(f'{BASE_URL}/users/me', tv)
    id_seller = id_retorno['id']
    return id_seller


def pegar_scroll_id(tv):
    url = f'{BASE_URL}/users/{id_conta(tv)}/items/search?search_type=scan&limit=100'
    resposta = fazer_reqs(url, tv)
    return resposta


def proxima_pagina(scroll, tv):
    url = f'{BASE_URL}/users/{id_conta(tv)}/items/search?search_type=scan&scroll_id={scroll}&limit=100'
    resposta = fazer_reqs(url, tv)
    return resposta
...
def pegar_todos_ids(tv):
    ...
    url = f'{BASE_URL}/users/{id_conta(tv)}/items/search?{filtro}&offset={0}'
    resposta = fazer_reqs(url, tv)
    ...
    if quantidade_de_an > 1000:
        ...
        for pagina in range(paginas):
            url = f'{BASE_URL}/users/{id_conta(tv)}/items/search?{filtro}&offset={pagina * 50}'
...
def atualizar(produto, valor_atualizar, tv, tipo, modo):
    url = f'{BASE_URL}/items/{produto}'
    info_produto = fazer_reqs(url, tv)
    vendedor = nome_conta(tv)
    tit_produto = info_produto['title']
    sku_produto = next(
            (atributo['values'][0]['name'] for atributo in info_produto['attributes'] if
             atributo['id'] == 'SELLER_SKU'), ''
            )
    tipo_envio = info_produto['shipping']['logistic_type']
    frete = 'Grátis' if info_produto['shipping']['free_shipping'] else 'Pago'
    status = Status[info_produto['status'].upper()].value if info_produto[
                                                                 'status'].upper() in Status.__members__ else Status.OTHER.value
    canal = 'Ambos' if len(info_produto['channels']) == 2 else info_produto['channels'][0].capitalize()
    msg_base = f'{status} | {produto} | {tipo_envio} | {frete} | {canal}'
    linha_ret = [vendedor, status, sku_produto, produto, tipo_envio, frete, canal, tit_produto]


    def retorno_linha(msg_ret, cor):
        msg_imprimir = f'{msg_base} | {msg_ret} | {tit_produto}'
        msg_dif(cor, '', msg_imprimir)
        linha_ret.append(msg_ret)
        return linha_ret


    if tipo == 'estoque':
        est_produto = info_produto['available_quantity']
        if valor_atualizar <= 0 and est_produto > 0 or valor_atualizar > 0 and est_produto <= 0:
            time.sleep(1)
        if tipo_envio == 'fulfillment':
            return retorno_linha('Estoque no Full', 'yellow')
        if valor_atualizar == est_produto:
            return retorno_linha('Estoque correto', 'white')
        payload = json.dumps(
                {'available_quantity': valor_atualizar, 'status': 'active' if valor_atualizar > 0 else 'paused'}
                )
        headers = {'Authorization': f'Bearer {tv}'}
        resposta = requests.put(url=url, headers=headers, data=payload)
        if resposta.status_code == 200:
            return retorno_linha(f'Estoque alterado de {est_produto} para {valor_atualizar}', 'green')
        else:
            return retorno_linha('Não pôde ser alterado', 'red')
        ...
        resposta = requests.put(url=url, headers=headers, data=payload)
        if resposta.status_code == 200:
            return retorno_linha(f'SKU novo: {valor_atualizar}', 'green')
        else:
            return retorno_linha('Não pôde ser alterado', 'red')
        ...
        novo_valor = valor_atualizar[1] if isinstance(valor_atualizar, list) and 'supermarket_eligible' in info_produto[
            'tags'] else valor_atualizar
        if preco_original_produto and float(novo_valor) < preco_produto:
            msg = (f'Pode ser vendido por: R$ {preco_imprimir(novo_valor)}. Desconto atual: R$ '
                   f'{preco_imprimir(preco_produto)} | {sku_produto} | {produto} | {tit_produto}')
            msg(msg)
            if tipo == 'desconto' and modo == 'auto_modificar':
                url = f'{BASE_URL}/seller-promotions/items/{produto}?promotion_type=PRICE_DISCOUNT&app_version=v2'
                requests.request('DELETE', url=url, headers={'Authorization': f'Bearer {tv}'})
                time.sleep(15)
                datas_desconto = pegar_datas()
                payload = json.dumps(
                        {
                            'deal_price'    : float(novo_valor),
                            'start_date'    : datas_desconto[0],
                            'finish_date'   : datas_desconto[1],
                            'promotion_type': 'PRICE_DISCOUNT'
                            }
                        )
                resposta = requests.request('POST', url=url, headers={'Authorization': f'Bearer {tv}'}, data=payload)
                if resposta.status_code == 201:
                    return retorno_linha(
                            f'Desconto recriado: De R$ {preco_imprimir(preco_produto)} por R$ {preco_imprimir(novo_valor)}',
                            'green'
                            )
                else:
                    return retorno_linha('Não pôde ser alterado', 'red')
            else:
                return retorno_linha(msg, 'white')
        elif preco_produto == novo_valor:
            return retorno_linha('Preço correto', 'white')
        else:
            if preco_produto < 79 and frete == 'Grátis':
                input('Produto com frete grátis abaixo de 79, por favor altere.\nAperte ENTER para continuar')
            if preco_produto >= 79 and novo_valor < 79:
                return retorno_linha('Não alterar: Desconto abaixo do valor de frete grátis', 'white')
            payload = json.dumps({'price': novo_valor}) if tipo == 'preço' else json.dumps(
                    {
                        'deal_price'    : float(novo_valor),
                        'start_date'    : pegar_datas()[0],
                        'finish_date'   : pegar_datas()[1],
                        'promotion_type': 'PRICE_DISCOUNT'
                        }
                    )
            resposta = requests.put(
                    url=url, headers={'Authorization': f'Bearer {tv}'}, data=payload
                    ) if tipo == 'preço' else requests.request(
                    'POST', url=f'{BASE_URL}/seller-promotions/items/{produto}?app_version=v2', headers={
                        'Authorization': f'Bearer {tv}'
                        }, data=payload
                    )
            if resposta.status_code == 200 if tipo == 'preço' else 201:
                return retorno_linha(
                        f'Preço alterado de R$ {preco_imprimir(preco_produto)} para R$ {preco_imprimir(novo_valor)}' if tipo == 'preço' else f'Desconto criado: De R$ {preco_imprimir(preco_produto)} por R$ {preco_imprimir(novo_valor)}',
                        'green'
                        )
            else:
                return retorno_linha('Não pôde ser alterado', 'red')
...
def pegar_produtos(sku, valor_atualizar, tv, tipo, modo):
    lista_feitos = []
    paginas = 0
    url = f'{BASE_URL}/users/{id_conta(tv)}/items/search?seller_sku={sku}&offset={paginas}'
    ...
    if quantidade_de_an == 0:
        ...
        if quantidade_de_an <= 50:
            ...
            for pagina in range(paginas):
                url = f'{BASE_URL}/users/{id_conta(tv)}/items/search?seller_sku={sku}&offset={(pagina * 50)}'
...
def obter_informacoes_vendas(tv, data_inicio, data_fim):
    url = f'{BASE_URL}/orders/search?seller={id_conta(tv)}&order.date_created.from={data_inicio}T00:00:00.000-00:00&order.date_created.to={data_fim}T23:59:59.999-00:00'
    headers = {'Authorization': f'Bearer {tv}'}
    resposta = requests.get(url, headers=headers)

    if resposta.status_code == 200:
        vendas = resposta.json()
        return vendas['results']
    else:
        msg_alerta('Falha ao obter informações de vendas')
        return []
...
def main():
    ...
    while not sair:
        ...
        if escolha == 'sair':
            ...
            filtro_4 = ''
            url_4 = f'{BASE_URL}/users/{id_conta(token)}/items/search?{filtro_4}&offset={0}'
            ...
            if planilha_atualizar != '':
                ...
                if continuar == '1':
                    ...
                    for inx_sku, lista_sku_value in enumerate(lista_sku):
                        ...
                        url_df = f'{BASE_URL}/users/{id_conta(token)}/items/search?seller_sku={sku_mlb}&offset={0}'
            ...
            url_cat = f'{BASE_URL}/highlights/MLB/category/{categoria}'
            ...
            for inx_cat, item in enumerate(retorno_cat):
                ...
                if tipo_produto == 'PRODUCT':
                    url_cat_2 = f'{BASE_URL}/products/{produto_cat}'

                else:
                    url_cat_2 = f'{BASE_URL}/items/{produto_cat}'
                ...
                resposta_cat = requests.get(f'{BASE_URL}/users/{seller_id_cat}', headers=headers_cat)
        ...
        elif escolha == '8' or escolha == 'informações de vendas':
            if token == '':
                token = configurar_conta()

            msg_cima('Insira a data de início (formato YYYY-MM-DD):')
            data_inicio = input('> ')
            msg_cima('Insira a data de fim (formato YYYY-MM-DD):')
            data_fim = input('> ')

            vendas = obter_informacoes_vendas(token, data_inicio, data_fim)

            if vendas:
                df_vendas = pd.DataFrame(vendas)
                df_vendas.to_excel('vendas.xlsx', index=False)
                msg_destaque('Informações de vendas exportadas para vendas.xlsx')
            else:
                msg_alerta('Nenhuma venda encontrada para o período especificado')
        else:
            print()
